...
from Code.MCTS4DM.refine import *
from Code.MCTS4DM.UCB import *
...
class BoundedPriorityQueue:
    ...
    def show_contents(self):
        ...
        print("show_contents")
        for (q, coverage, entry_count, element,  _) in self.values:
            print(q, coverage, entry_count, element)
...
class MCTS4DM:
    ...
    def __init__(self, df, target_column='target',
        ...
        self.df = df
        self.df_descriptives = self.df.drop(columns=[self.target_column])
    ...
    def simulate(self, node):
        ...
        if self.reward_policy == 'random_pick': 
            ...
            k_placeholder = 0 
            sum_reward = 0
            ranked_path = [(-item.quality, item) for item in path]
            max_heap = heapq.heapify(ranked_path)
    ...
    def run(self):
        ...
        self.result = self.result_set.get_values()
    ...
    def run_statistical_test(self, k, show_progress_pvals=False, save_path='saved_results_statistical_test_mcts4dm_'):
        ...
        for _ in range(k):
            if show_progress_pvals:
                print('Start mcts4dm statistical test: ',_,'/',k)
            swapped_matrix = swap_randomize_symmetric(self.matrix)
            MCTS4DM_swapped = MCTS4DM(self.df, target_column=self.target_column,
                                        q=1,
                                        root_description=self.root_description,
                                        n_chunks=self.n_chunks,
                                        allow_exclusion=self.allow_exclusion,
                                        minutes=self.minutes,
                                        max_nr_iterations = self.max_nr_iterations,
                                        ucb_type = self.ucb_type,  # or 'UCB1', 'UCB1-Tuned', 'SP-MCTS', 'UCT, 'DFS-UCT'
                                        ucb_params = self.ucb_params,
                                        quality_params = self.quality_params,
                                        matrix = swapped_matrix,
                                        size_correction_method = self.size_correction_method,
                                        max_desc_length = self.max_desc_length,
                                        min_coverage = self.min_cov,
                                        roll_out_strategy = self.roll_out_strategy,  # or 'large-freq' or 'naive'
                                        roll_out_jump_length = self.roll_out_jump_length,  # if >1 then roll-out-strategy = 'large-freq'
                                        reward_policy = self.reward_policy,  # or 'random_pick', 'mean_path', 'mean_top_k'
                                        reward_policy_k = self.reward_policy_k,
                                        memory_policy = self.memory_policy,  # or 'top_k'
                                        memory_policy_k_value = self.memory_policy_k_value,
                                        update_policy = self.update_policy,  # or 'mean_update', 'top_k_mean_update'
                                        update_policy_k = self.update_policy_k,
                                        diversity_policy = self.diversity_policy,
                                        show_progress = self.show_progress,
                                        show_result = self.show_result,
                                        )
            MCTS4DM_swapped.run()
            randomized_qualities += MCTS4DM_swapped.qualities
