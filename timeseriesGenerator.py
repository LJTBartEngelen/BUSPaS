...
# nr_inst = 100  # number of  records
#
# # target variables
# mu = 50
# mu_err = 0  # white noise mean
# sig_err = 1.5  # white noise std
# season = 4  # nr of seasons in period
# alpha = 0.8  # contribution of seasonality compared to not seasonality
# nr_tp = 12 * season  # number of periods * seasons in period
#
# # descriptive attributes
# nr_attr = 5  # number of attributes
# cats_low = 2  # aantal categorien voor een attribute random gekozen tussen deze waarde
# cats_high = 8  # en deze waarde
...
def synthetic_data_generator(nr_inst=100, nr_attr=5, cats_low=2, cats_high=10, nr_tp=48, mu=50, mu_err=0, sig_err=1,
    ...
    for i in range(nr_inst):
        ...
        try:
            ...
            #TODO change? ts_random_chauchy_shift_prod // ts_random_wn_shift // ts_random_chauchy_shift_prod
        data.append(r)
...
def test_subgroup_generator(df, mu, mu_err, sig_err, alpha, season, size_new_sg=50, descr='auto',**kwargs):
    """builds a test subgroup based on parameter values and columns in the existing dataframe, outputs the subgroup in list"""
    ...
    new_sg = []
    for i in range(size_new_sg):  # TODO change way in which distribution
...
def create_true_sg(df, true_subgroup_attributes, sg_format, deviation=0.5, outliers=0, true_sg_type="error_shift"):
    ...
    new_ts = pd.DataFrame(sg, columns=df.columns, index=true_sg.index)  # .iloc[:,-1]
